...
class resnetv1(Network):
    ...
    def _crop_pool_layer(self,bottom,rois,name):
        with tf.variable_scope(name) as scope:
            batch_inds = tf.squeeze(tf.slice(rois,[0,0],[-1,1],name='batch_id'),[1])
            # 获取正则化的边界框坐标
            ...
            if cfg.RESNET.MAX_POOL:
                ...
                crops = tf.image.crop_and_resize(bottom,bboxes,tf.to_int32(batch_inds),[pre_pool_size,pre_pool_size],
                                                 name='crops')
                crops = slim.max_pool2d(crops,[2,2],padding='SAME')
            else:
                crops = tf.image.crop_and_resize(bottom,bboxes,tf.to_int32(batch_inds),[cfg.POOLING_SIZE,cfg.POOLING_SIZE],
                                                 name='crops')
        return crops
