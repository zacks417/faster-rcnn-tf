...
class pascal_voc(imdb):
    def __init__(self,image_set,year,use_diff=False):
        name = 'voc_' + str(year) + '_' + image_set
        if use_diff:
            name += '_diff'
        imdb.__init__(self,name)
        self._year = year
        self._image_set = image_set
        # 'D:\Faster_R-CNN\data\VOCdevkit2007'
        self._devkit_path = self._get_default_path()
        # 数据库路径'D:\Faster_R-CNN\data\VOCdevkit2007\VOC2007'
        ...
        self._roidb_handler = self.gt_roidb # 感兴趣区域数据库
        # 生成唯一ID，基于随机数
